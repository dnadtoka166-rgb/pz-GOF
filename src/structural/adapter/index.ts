...
import { GlovoFoodAdapter } from './GlovoAdapter';
import { UklonFoodAdapter } from './Uklon';
...
const glovo = new GlovoFoodAdapter();
placeOrder(glovo, 'Американо');
placeOrder(glovo, 'Еспресо');

const uklon = new UklonFoodAdapter();
placeOrder(uklon, 'Мокачино');
placeOrder(uklon, 'Флет Вайт');
